...
public class ChessClient extends Application {
	...
	public void start(Stage primaryStage) {
		primaryStage.setTitle("CS 76 Chess Mau");
		...
		moveMaker[Chess.BLACK] = new AIMoveMaker(new ChessMinimaxAI());
	...
	private class GameHandler implements EventHandler<ActionEvent> {
		...
		public void handle(ActionEvent e) {
			...
			if (mover.getState() == Worker.State.READY) {
				...
				if (move == (short) 0 && game.position.getAllMoves().length == 0){
		...
		private void logPly(Position position, short move) throws IllegalMoveException {
			String plyNumber = (position.getPlyNumber() + 1) + ". ", 
					moveDescription = Move.getString(move).toUpperCase(),
					winning = "";
			...
			if (move != Move.NO_MOVE) {
				...
				if (position.getAllMoves().length == 0) {
					...
					Double eval = position.getMaterial() + position.getDomination() / 5;
					eval *= position.getToPlay() == 0 ? -1 :1; 	
					
					if (eval.equals(-0.42)) {
						winning = "EQUALLY";
					} else {
						winning += eval > -0.42 ? "BLACK MORE" : "WHITE MORE";
					}
				}
				
				position.undoMove();
				
				logArea.replaceSelection(plyNumber + moveDescription + " " + winning + " LIKELY TO WIN\n");
			}
		}
	}
